...
class _BookingsPageState extends State<BookingsPage> {
  ...
  Future<void> _generateInvoicePdf(Map<String, dynamic> booking) async {
    ...
    final totalAmount = double.tryParse(booking['totalAmount'].toString()) ?? 0.0;
    final advancePaid = double.tryParse(booking['advancePaid'].toString()) ?? 0.0;
        ...
        pw.Column(
            ...
            children: [
                ...
                child: pw.Row(
                  ...
                  children: [
                    pw.Column(
                      crossAxisAlignment: pw.CrossAxisAlignment.start,
                      children: [
                        pw.Text("HOTEL BOOKING INVOICE", style: pw.TextStyle(fontSize: 24, fontWeight: pw.FontWeight.bold, color: PdfColors.white)),
                        ...
                        pw.Text(
                          ...
                          style: pw.TextStyle(
                            fontSize: 12,
                            color: PdfColor(1, 1, 1, 0.8), // white with 80% opacity
                          ),
                        ),
                      ],
                    ),
                    pw.Text("#$invoiceId", style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, color: accentColor)),
                  ],
                ),
              ),
              ...
              pw.Row(
                ...
                children: [
                  // Hotel Details
                  _buildAddressBlock("Billed By:", hotelName, booking['hotelAddress'], primaryColor),
                  // Invoice Details
                  _buildAddressBlock("Invoice Date:", "Date: ${invoiceDate.toLocal().toString().split(' ')[0]}", "Contact: ${booking['hotelPhone']}", primaryColor, alignRight: true),
                ],
              ),
              ...
              pw.Row(
                  ...
                  children: [
                    pw.Column(
                      crossAxisAlignment: pw.CrossAxisAlignment.start,
                      children: [
                        pw.Divider(color: PdfColors.grey500, thickness: 1, height: 10),
                    ...
                    pw.Column(
                      ...
                      children: [
                        pw.Divider(color: primaryColor, thickness: 1, height: 10),
                        pw.Text("Hotel Management Signature", style: pw.TextStyle(fontSize: 10, color: primaryColor)),
                      ],
                    ),
                  ]
              ),
  ...
  pw.Widget _infoSection(String title, Map<String, String> data, PdfColor headerColor) {
    return pw.Container(
                ...
                return pw.TableRow(
                  children: [
                    pw.Padding(
                      ...
                      child: pw.Text("${e.key}:", style: pw.TextStyle(fontWeight: pw.FontWeight.bold, color: PdfColors.grey700)),
                    ),
                    pw.Padding(
                      padding: const pw.EdgeInsets.symmetric(vertical: 4),
                      child: pw.Text(e.value, style: const pw.TextStyle(fontSize: 12)),
                    ),
                  ],
                );
              }).toList(),
  ...
  pw.Widget _buildAddressBlock(String title, String line1, String line2, PdfColor color, {bool alignRight = false}) {
    return pw.Container(
      width: 200,
      child: pw.Column(
        crossAxisAlignment: alignRight ? pw.CrossAxisAlignment.end : pw.CrossAxisAlignment.start,
        children: [
          pw.Text(title, style: pw.TextStyle(fontSize: 12, color: PdfColors.grey700, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 5),

          // Use the non-nullable `color` parameter
          pw.Text(line1, style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold, color: color)),
          pw.Text(line2, style: const pw.TextStyle(fontSize: 12, color: PdfColors.grey600)),
        ],
      ),
    );
  }
        ...
        children: [
          ...
          pw.Container(
            ...
            child: pw.Row(
              mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
              children: [
                pw.Text("PAYMENT SUMMARY", style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold, color: PdfColors.white)),
              ],
            ),
          ),

          // Data Rows
          _buildSummaryRow("Total Booking Amount", "\$${totalAmount.toStringAsFixed(2)}", isTotal: false, color: primaryColor),
          _buildSummaryRow("Advance Paid", "\$${advancePaid.toStringAsFixed(2)}", isTotal: false, color: accentColor),
          _buildSummaryRow("BALANCE DUE", "\$$dueAmount", isTotal: true, color: accentColor),
        ],
      ),
    );
  }
  ...
  pw.Widget _buildSummaryRow(String label, String value, {bool isTotal = false, PdfColor? color}) {
    ...
    return pw.Container(
      ...
      decoration: pw.BoxDecoration(
        ...
        color: rowBgColor,
        border: pw.Border(bottom: const pw.BorderSide(color: PdfColors.grey200, width: 0.5)),
      ),
      child: pw.Row(
        mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
        children: [
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildSectionTitle("Contact Person Details", Icons.contact_mail_rounded, kBookingPrimary),
                _inputField("Contract Person", contactPersonCtrl, required: true).animate().slideX(duration: 300.ms, begin: -0.1),
                _inputField("Phone", contactPhoneCtrl, required: true).animate().slideX(duration: 300.ms, begin: 0.1),
                const SizedBox(height: 15),

                // --- Section 2: Booking Summary & Final Inputs ---
                _buildSectionTitle("Booking Summary & Final Payment", Icons.info_rounded, kBookingAccent),
                _buildReadOnlySummary().animate().fadeIn(duration: 500.ms, delay: 300.ms), // Contains all remaining fields
                const SizedBox(height: 20),

                // --- Action Button ---
                isLoading
                    ? Center(child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(kBookingPrimary)))
                    : ElevatedButton.icon(
                ...
                ).animate().scale(duration: 400.ms, delay: 600.ms, curve: Curves.easeOutBack),
              ],
            ),
          ),
        ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _inputField("Customer Name", nameCtrl, readOnly: true, color: kBookingAccent),
          _inputField("Email", emailCtrl, readOnly: true),
          _inputField("Phone", phoneCtrl, readOnly: true),
          // Hotel Details (Read-Only)
          _inputField("Hotel Name", hotelNameCtrl, readOnly: true, color: kBookingAccent),
          ...
          _inputField("Number of Rooms", numRoomsCtrl, required: true, color: kBookingPrimary),
          _inputField("Advance Paid (\$)", advanceCtrl, required: true, color: kBookingPrimary),

          // --- Final Calculated Amounts (Read-Only) ---
          _inputField("Total Amount", totalCtrl, readOnly: true, color: kBookingAccent),
          _inputField("Due Amount", dueAmountCtrl, readOnly: true, color: kBookingAccent),
        ],
      ),
    );
  }
  ...
  Widget _inputField(String label, TextEditingController ctrl, {bool readOnly = false, bool required = false, Color? color}) {
    ...
    return Padding(
      ...
      child: TextFormField(
        controller: ctrl,
        readOnly: readOnly,
        keyboardType: (label.contains("Number") || label.contains("Amount") || label.contains("Paid") || label.contains("Price"))
